...
双击即可。自动完成：启动 proxy(:1800) + 启动 UI(:8090) + 打开浏览器，并为当前用户注册 Windows 登录自启。

```bat
helmx startup status
helmx startup on
helmx startup off
```

登录自启使用 `--autostart` 隐藏窗口运行，不会自动打开浏览器。proxy 就绪后会自动运行一次 `helmx` 激活自检，验证页会从“等待自动激活”更新为运行结果；该检查创建隔离的 `codex exec` 会话，仅用于确认注入生效。
