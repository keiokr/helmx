...
helmx startup on      :: Enable per-user Windows login startup
helmx startup off     :: Disable per-user Windows login startup
```

Double-clicking the executable enables per-user login startup. The startup path
runs hidden and does not open the browser. After the proxy is ready, helm-x
automatically runs one isolated `codex exec helmx` activation check and reports
the result in the verification page.
