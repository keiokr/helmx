...
cmake --fresh -S . -B build -G "MinGW Makefiles" -DCMAKE_BUILD_TYPE=Release || goto :error
cmake --build build -j || goto :error

REM 3. optional UPX compress (smallest practical single exe)
if exist "C:\Tools\upx\upx.exe" (
    "C:\Tools\upx\upx.exe" --best --lzma --overlay=strip build\helmx.exe >nul 2>&1
