...
// Sanity-check TOML before overwriting a user's config.  This is intentionally
// lenient: real-world Codex configs contain multi-line arrays (e.g.
// notify = [...]), single-quoted strings, numbers and booleans, so we only
// reject clearly broken table headers / double-quoted strings, and otherwise
// tolerate the rest.
static bool toml_valid_content(const std::string& content) {
    std::istringstream lines(content);
    std::string line;
    int array_depth = 0;  // inside a multi-line [...] value when > 0
    while (std::getline(lines, line)) {
        size_t first = line.find_first_not_of(" \t\r");
        if (first == std::string::npos || line[first] == '#') continue;

        if (array_depth > 0) {
            // Consume lines inside a multi-line array until brackets balance.
            for (char c : line) {
                if (c == '[') ++array_depth;
        ...
        if (line[first] == '[') {
            ...
            if (close == std::string::npos) {
                // No closing bracket on this line: could be the start of a
                // multi-line array value. Count brackets to decide.
                int depth = 0;
                for (char c : line) {
                    if (c == '[') ++depth;
                    else if (c == ']') --depth;
                }
                if (depth > 0) array_depth = depth;
                continue;
            }
            if (close == first + 1) return false;
        ...
        if (eq == std::string::npos) continue;  // array items / continuation lines
        if (eq == first) return false;
        size_t value = line.find_first_not_of(" \t", eq + 1);
        if (value == std::string::npos) return false;
        if (line[value] == '[') {
            // Value is an array; if it spans lines, track bracket depth.
            int depth = 0;
            for (size_t i = value; i < line.size(); ++i) {
                if (line[i] == '[') ++depth;
...
bool inject_config(const std::string& home) {
    ...
    if (!fs::exists(cfg)) {
        std::fprintf(stderr, "[helm-x] 未找到 config.toml: %s\n", cfg.string().c_str());
        return false;
    }
    if (!backup_config(cfg.string())) {
        std::fprintf(stderr, "[helm-x] 备份失败\n");
        return false;
    }
...
bool restore_config_proxy(const std::string& home, const std::string& fallback_url) {
    fs::path cfg = fs::path(home) / "config.toml";
    fs::path bak = cfg.string() + ".helmx-proxy-bak";

    // Only replace the active provider's local base_url. All unrelated Codex
    // settings remain exactly as they are in the live config.
    std::string current;
    if (!read_file(cfg, current) || !toml_valid_content(current)) return false;

    std::string provider;
    std::string current_url;
    if (!active_provider(current, provider) ||
        !provider_base_url(current, provider, current_url)) return false;
    ...
    if (fs::exists(bak)) {
        std::string backup;
        if (read_file(bak, backup) && toml_valid_content(backup)) {
            provider_base_url(backup, provider, original_url);
        }
    }
    if (original_url.empty() &&
        !fallback_url.empty() &&
        fallback_url.find("127.0.0.1") == std::string::npos) {
        original_url = fallback_url;
    }
    if (original_url.empty()) return false;

    if (!replace_provider_base_url(current, provider, original_url) ||
        !atomic_write(cfg, current)) return false;
...
bool deploy_agents(const std::string& home) {
    ...
    if (content.empty()) {
        std::fprintf(stderr, "[helm-x] AGENTS.md 资源为空\n");
        return false;
    }
...
int apply() {
    ...
    std::printf("[helm-x] codex 主目录: %s\n", home.c_str());

    bool ok = true;
    ok &= inject_config(home);
    // AGENTS.md 不部署为文件 — 由代理从加密资源注入
    ok &= verify_injection(home);

    if (ok) {
        std::printf("[helm-x] 应用成功。运行: helmx activate  (发送激活词)\n");
        return 0;
    }
    std::fprintf(stderr, "[helm-x] 应用时出现错误\n");
    return 1;
}
