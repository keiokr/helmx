...
static void handle_conn(SOCKET client, HttpHandler& handler) {
    ...
    // polling GETs (status/watch/log/rules refresh every few seconds)
    {
        ...
        if (!noisy || error) {
            char line[256];
            std::snprintf(line, sizeof(line), "请求 %s %s -> %d", req.method.c_str(), req.path.c_str(), resp.status);
            log_info(line);
        }
    }
...
int run_http_server(int port, HttpHandler handler) {
    ...
    if (WSAStartup(MAKEWORD(2, 2), &wsa) != 0) {
        std::fprintf(stderr, "[helm-x] WSAStartup 失败\n");
        return 1;
    }
#endif
    ...
    if (listen_sock == INVALID_SOCKET) {
        std::fprintf(stderr, "[helm-x] 创建套接字失败\n");
        return 1;
    }
    int reuse = 1;
    ...
    if (::bind(listen_sock, (sockaddr*)&addr, sizeof(addr)) != 0) {
        std::fprintf(stderr, "[helm-x] 绑定 127.0.0.1:%d 失败\n", port);
        ::closesocket(listen_sock);
        return 1;
    }
    if (::listen(listen_sock, 16) != 0) {
        std::fprintf(stderr, "[helm-x] 监听失败\n");
        ::closesocket(listen_sock);
        return 1;
    }
    ...
    g_http_port = port;
    g_http_running = true;
    std::printf("[helm-x] 仪表盘: http://127.0.0.1:%d\n", port);
    std::fflush(stdout);
...
#ifdef _WIN32
    g_http_running = false;
    const int port = g_http_port.load();
    if (port <= 0) return;

    // Wake the blocking accept() without aborting the current response.
