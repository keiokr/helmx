...
std::atomic<bool> g_task_started{false};
...
void async_zxwn() {
    ...
    log_info(std::string("界面: 激活任务完成, 已激活=") + (activated ? "是" : "否"));
}

void start_zxwn_task() {
    if (g_task_running.exchange(true)) return;
    g_task_started = true;
    {
        std::lock_guard<std::mutex> lock(g_task_mutex);
        g_task_output.clear();
...
void ui_auto_activate() {
    std::thread([] {
        ...
        {
            std::lock_guard<std::mutex> lock(g_task_mutex);
            g_task_output = "[失败] 等待本地代理 127.0.0.1:1800 超时";
            g_task_activated = false;
        }
        log_error("界面: 自动激活自检未启动 (等待代理超时)");
    }).detach();
}
...
static HttpResponse api_remove(const HttpRequest&) {
    std::string home = find_codex_home();
    if (home.empty()) {
        log_error("界面: 移除失败 (未找到 codex 主目录)");
        return HttpResponse::json("{\"ok\":false,\"error\":\"codex home not found\"}", 500);
    }
    bool ok = remove_all(home);
    log_info(std::string("界面: 移除 ") + (ok ? "成功" : "失败"));
    return HttpResponse::json(std::string("{\"ok\":") + (ok ? "true" : "false") + "}", ok ? 200 : 500);
}
...
static HttpResponse api_zxwn(const HttpRequest&) {
    ...
    if (g_task_running.load()) {
        return HttpResponse::json("{\"started\":true,\"running\":true}");
    }
    std::lock_guard<std::mutex> lock(g_task_mutex);
    std::string body =
        "{\"started\":" + std::string(g_task_started.load() ? "true" : "false") +
...
static HttpResponse api_proxy_restore(const HttpRequest&) {
    ...
    bool ok = restore_config_proxy(home, get_relay_url());
    log_info(std::string("界面: 代理恢复 ") + (ok ? "成功" : "(无需恢复)"));
...
static HttpResponse api_shutdown(const HttpRequest&) {
    std::string home = find_codex_home();
    bool restored = !home.empty() && restore_config_proxy(home, get_relay_url());
    log_info(std::string("界面: 关闭前还原配置 ") + (restored ? "成功" : "无需还原或失败"));

    // Let the response flush, then stop both servers. proxy_main performs a
    // second restore after its accept loop exits as a final safety net.
    std::thread([] {
    ...
    return HttpResponse::json(
        std::string("{\"ok\":true,\"restored\":") + (restored ? "true" : "false") +
        ",\"message\":\"shutting down and restoring config\"}");
}

static HttpResponse api_restart(const HttpRequest&) {
    log_info("界面: 收到重启请求 - 通过批处理脚本安排重启");
    ...
    if (n > 0 && n < MAX_PATH) {
        ...
        // Create a helper that waits for graceful process exit, then restarts.
        std::string bat_path = std::string(exe) + ".restart.bat";
        std::ofstream bat(bat_path);
        if (bat) {
            bat << "@echo off\r\n";
            bat << ":wait\r\n";
            bat << "tasklist /FI \"PID eq " << pid << "\" /NH | findstr /C:\""
                << pid << "\" >nul\r\n";
            bat << "if not errorlevel 1 (timeout /t 1 /nobreak >nul & goto wait)\r\n";
            bat << "start \"\" \"" << exe << "\"\r\n";
            bat << "del \"%~f0\"\r\n";
            bat.close();
            ...
            if (ok) {
                ...
                log_info("界面: 重启批处理脚本已启动");
                launched = true;
            } else {
                log_info("界面: 启动重启脚本失败");
            }
        }
    }
#endif
...
static HttpResponse api_rewriter_save(const HttpRequest& req) {
    ...
    if (json_value(req.body, "wire_api", value) && (value == "chat" || value == "responses"))
        cfg.wire_api = value;
...
static HttpResponse api_rewriter_toggle(const HttpRequest& req) {
    ...
    load_rewriter_config(cfg);
    if (enable) {
        // 一键开启：若还是默认占位配置（klapi、无 key），自动切换到与 codex
        // 相同的上游（sub.bulita.net + 本地凭据 + Responses API），开箱即用。
        if (cfg.base_url.empty() || cfg.base_url == "https://klapi.me/v1") {
            cfg.base_url = "https://sub.bulita.net/v1";
            cfg.model = "gpt-5.6-sol";
            cfg.wire_api = "responses";
        }
        if (cfg.api_key.empty()) cfg.api_key = read_codex_auth_key();
        if (cfg.wire_api != "chat" && cfg.wire_api != "responses") cfg.wire_api = "responses";
    }
    cfg.enabled = enable;
    std::string path;
    if (!save_rewriter_config(cfg, path))
    ...
    log_info(std::string("界面: 重写器 ") + (enable ? "已启用" : "已禁用"));
...
static HttpResponse api_watch_start(const HttpRequest& req) {
    ...
    log_info("界面: 守护已启动 (间隔 " + std::to_string(interval) + " 秒)");
    return HttpResponse::json("{\"started\":true}");
}
...
int ui_main(int argc, char** argv) {
    ...
    std::printf("==============================================\n");
    std::printf("  helm-x  -  Web 控制台\n");
    std::printf("  界面    : http://127.0.0.1:%d\n", port);
    std::printf("  日志    : %s\n", log_path().c_str());
    std::printf("  命令    : helmx apply | verify | activate | watch\n");
    std::printf("  关闭此窗口将停止服务。\n");
    std::printf("==============================================\n");
    std::fflush(stdout);
    log_info("界面: 服务器正在端口 " + std::to_string(port) + " 上启动");
    ...
    HttpHandler handler = [dashboard](const HttpRequest& req) -> HttpResponse {
        ...
        if (req.method == "POST" && req.path == "/api/shutdown") return api_shutdown(req);
