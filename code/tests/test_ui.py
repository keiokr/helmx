...
class TestUi(unittest.TestCase):
    def _proxy_env(self):
        """Isolated CODEX_HOME so proxy tests never touch the real ~/.codex
        config (helmx proxy auto-injects base_url into config.toml)."""
        tmp = tempfile.TemporaryDirectory(prefix="helmx-proxy-")
        ...
        return tmp, {**os.environ, "CODEX_HOME": str(codex_home)}

    def test_dashboard_defaults_to_self_check_above_overview(self):
        dashboard = (
            Path(__file__).resolve().parents[1] / "assets" / "dashboard.html"
        ).read_text(encoding="utf-8")
        verify_button = '<button class="nav-button active" data-view="verify">'
        overview_button = '<button class="nav-button" data-view="dashboard">'
        self.assertIn(verify_button, dashboard)
        self.assertIn(overview_button, dashboard)
        self.assertLess(dashboard.index(verify_button), dashboard.index(overview_button))
        self.assertIn('<span id="page-title">自检</span>', dashboard)
        self.assertIn('<section class="view active" id="view-verify">', dashboard)
        self.assertIn('<section class="view" id="view-dashboard">', dashboard)
        self.assertIn("loadQa(false); runVerify();", dashboard)

    @unittest.skipUnless(os.name == "nt", "restore guard is Windows-specific")
    def test_force_killed_proxy_is_restored_by_guard(self):
        tmp, env = self._proxy_env()
        codex_home = Path(env["CODEX_HOME"])
        cfg = codex_home / "config.toml"
        backup = codex_home / "config.toml.helmx-proxy-bak"
        port = free_port()
        ...
        try:
            ...
            while time.time() < deadline:
                if backup.exists() and f"http://127.0.0.1:{port}/v1" in cfg.read_text(
                    encoding="utf-8"
                ):
                    break
                time.sleep(0.1)
            else:
                self.fail("proxy did not inject config before timeout")

            # The runtime upstream is sufficient for recovery even if the
            # sidecar backup is missing or was removed unexpectedly.
            backup.unlink()
            proc.kill()
            proc.wait(timeout=5)
            ...
            while time.time() < deadline:
                ...
                if "https://example.com/v1" in text and not backup.exists():
                    break
                time.sleep(0.1)
            else:
                self.fail("restore guard did not restore config after forced termination")
        finally:
    ...
    def test_proxy_forwards_codex_identity_headers(self):
        ...
        tmp, proxy_env = self._proxy_env()
        proc = subprocess.Popen([
            str(HELMX), "proxy", "--listen", str(proxy_port),
            "--upstream", f"http://127.0.0.1:{upstream.server_port}/v1",
        ], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, env=proxy_env)
        headers = {
    ...
    def test_rewriter_save_preserves_existing_key(self):
        with tempfile.TemporaryDirectory(prefix="helmx-ui-") as tmp:
            appdata = Path(tmp) / "AppData" / "Roaming"
            appdata.mkdir(parents=True)
            # config.toml inside CODEX_HOME keeps find_codex_home() (and the
            # helmx.log) inside the temp dir instead of the real ~/.codex.
            codex_home = Path(tmp) / ".codex"
            codex_home.mkdir()
            (codex_home / "config.toml").write_text(
                'model_provider = "test"\n[model_providers.test]\nbase_url = "https://example.com/v1"\n',
