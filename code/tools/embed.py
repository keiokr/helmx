...
def main():
    # 让控制台按 UTF-8 输出，避免中文文件名在 GBK 控制台下乱码（鈥?）。
    if os.name == 'nt':
        try:
            import ctypes
    ...
    auth_context = read('auth-context.md')
    ...
    auth_context = norm(auth_context)
    rules = norm(rules)
    ...
    emit_var('kAuthContext', auth_context, KEY_SEED + 0x9000)
    ...
    print(f'  auth-context.md: {len(auth_context)} bytes -> cipher')
